...
private:
  ...
  const QDir m_redwindDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfRedwindBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _coin) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo redwindBlockchainInfo() const;
